...
from architecture import S3D # TSN using Inception backbone
from evaluation import MAPs

class DCH_S3D(object):
    def __init__(self, config):
        ...
        for k, v in vars(config).items():
            setattr(self, k, v)
        # try nlbs
        self.file_name = 'lr_{}_cqlambda_{}_alpha_{}_bias_{}_gamma_{}_dataset_{}_s3d'.format(
                self.lr,
                self.q_lambda,
                self.alpha,
                self.bias,
        ...
        configProto = tf.ConfigProto(device_count = {'GPU': 1})
        ...
        self.vid = tf.placeholder(tf.float32, [None, 12, 256+28, 256+28, 3]) # 20190708 trial
        ...
        self.global_step = tf.Variable(0, trainable=False) # reuse ?
        self.train_op = self.apply_loss_function(self.global_step)
        run_options = tf.RunOptions(report_tensor_allocations_upon_oom=True) # added 20190625
        self.sess.run(tf.global_variables_initializer())#, options=run_options)
        return

    def load_model(self):
        # print('Just tracking...') # after Ln42
        if self.img_model == 's3d': # from args
            img_output = S3D(
                    self.vid,
                    self.batch_size,
                    self.output_dim,
    ...
    def cauchy_cross_entropy(self, u, label_u, v=None, label_v=None, gamma=1, normed=True): # pairwise
        # print('label_u', label_u) # <tf.Tensor 'Placeholder_1:0' shape=(?, 80) dtype=float32>
        if v is None:
            v = u
            label_v = label_u
        ...
        cauchy = gamma / (dist + gamma) # Cauchy distribution
        ...
        mask = tf.equal(tf.eye(tf.shape(u)[0]), tf.constant(0.0)) # 64X64  tf.boolean_mask(tensor, mask, axis)
        
        cauchy_mask = tf.boolean_mask(cauchy, mask) # return _apply_mask_1d(tensor, mask, axis) # return gather(param, indices)
    ...
    def apply_loss_function(self, global_step):
        ...
        opt = tf.train.MomentumOptimizer(learning_rate=lr, momentum=0.9) # reuse issue
        # compute_gradients(a, b) returns d[sum a]/d[b]
        grads_and_vars = opt.compute_gradients(self.loss, self.train_layers+self.train_last_layer) # exploit...
        
        fcgrad, _ = grads_and_vars[-2] # fc8-w
        fbgrad, _ = grads_and_vars[-1] # fc8-b
        ...
        print('++', len(self.train_layers), len(self.grads_and_vars)) # ('++', 28, 30)
        for i in range(len(self.grads_and_vars)):
            print( "{0} {1}\n".format(i, self.grads_and_vars[i][0]) )
        if self.finetune_all:
            return opt.apply_gradients([(grads_and_vars[0][0], self.train_layers[0]),
                                        ...
                                        (grads_and_vars[14][0], self.train_layers[14]),
                                        (grads_and_vars[15][0]*2, self.train_layers[15]),
                                        (grads_and_vars[16][0], self.train_layers[16]),
                                        (grads_and_vars[17][0]*2, self.train_layers[17]),
                                        (grads_and_vars[18][0], self.train_layers[18]),
                                        (grads_and_vars[19][0]*2, self.train_layers[19]),
                                        (grads_and_vars[20][0], self.train_layers[20]),
                                        (grads_and_vars[21][0]*2, self.train_layers[21]),
                                        (grads_and_vars[22][0], self.train_layers[22]),
                                        (grads_and_vars[23][0]*2, self.train_layers[23]),
                                        (grads_and_vars[24][0], self.train_layers[24]),
                                        (grads_and_vars[25][0]*2, self.train_layers[25]),
                                        (grads_and_vars[26][0], self.train_layers[26]),
                                        (grads_and_vars[27][0]*2, self.train_layers[27]),
    ...
    def train(self, img_dataset):
        ...
        for train_iter in range(self.iter_num):
            ...
            #print('++ len of labels', len(labels), labels.shape) # 64, (64 output_dim, 80 categories)
            start_time = time.time()
            # loss = cos_loss + q_loss
            _, loss, cos_loss, output, summary = self.sess.run([self.train_op, self.loss, self.cos_loss, self.img_last_layer, self.merged],
                                    feed_dict={self.vid: images,
