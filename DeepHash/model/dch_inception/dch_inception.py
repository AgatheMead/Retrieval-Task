...
from architecture import temporal_segment_layers # TSN using Inception backbone
from evaluation import MAPs


class DCH_Inception(object):
    def __init__(self, config):
        ...
        for k, v in vars(config).items():
            setattr(self, k, v)
        # try nlbs
        self.file_name = 'lr_{}_cqlambda_{}_alpha_{}_bias_{}_gamma_{}_dataset_{}_nlb2'.format(
                self.lr,
                self.q_lambda,
                self.alpha,
                self.bias,
        ...
        configProto = tf.ConfigProto(device_count = {'GPU': 1}) # pending cannot link to GPU-1
    ...
    def load_model(self):
        ...
        if self.img_model == 'tsn': # from args
            img_output = temporal_segment_layers(
                    self.img,
                    self.batch_size,
                    self.output_dim,
                    self.stage,
    ...
    def apply_loss_function(self, global_step):
        ...
        grads_and_vars = opt.compute_gradients(self.loss, self.train_layers+self.train_last_layer) # len(grads_and_vars)==16
        ...
        #print('++', len(self.train_layers), len(self.grads_and_vars)) # ('++', 14, 16)
