...
from architecture import img_vggnet_layers
from evaluation import MAPs


class DCH_VGG(object):
    ...
    def load_model(self):
        ...
        if self.img_model == 'vgg16': # from args
            img_output = img_vggnet_layers(
                    self.img,
                    self.batch_size,
                    self.output_dim,
                    self.stage,
    ...
    def cauchy_cross_entropy(self, u, label_u, v=None, label_v=None, gamma=1, normed=True):
        ...
        s = tf.clip_by_value(label_ip, 0.0, 1.0) # clip the values between 0-1
        ...
        if normed:
            ...
            dist = r_u - 2 * tf.matmul(u, tf.transpose(v)) + \
                tf.transpose(r_v) + tf.constant(0.001) # Hamming distance
